...
export async function trackUniqueMintData(
	...
	try {
		...
		const metadataUri = api.registry
...
export async function trackTokenSeriesData(
	...
	try {
		...
		const seriesId = checkIfSingleTokenInSeries
			? eventData[1][1]
			: eventData[1];
		...
		const imageIndex =
			params.findIndex(
				(p) =>
					p.name === "metadataPath" ||
					p.type === "CrmlNftMetadataScheme" ||
					p.type === "MetadataScheme" ||
...
export async function trackAdditionalTokenData(
	...
	try {
		...
		const [_nextSerialNumber, _metadataUri] = await Promise.all([
		...
		const metadataUri = _metadataUri.toHuman();
